...
import { MOCK_MEMBERS, cognitiveBadge, timeAgoKo, type Member } from '@/lib/members';
...
export default function MembersPage() {
  ...
  function handleCreated(m: Member) {
  ...
  return (
    <>
      <PageHeader
        title="회원 관리"
        description="어르신과 보호자 정보를 등록하고 관리합니다."
        actions={
          <button
      ...
      <Card className="px-4 py-3">
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-lg bg-slate-50 ring-1 ring-slate-100 text-[13px] text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300"
          />
          <div className="flex gap-2">
            <select
              value={cogFilter}
              onChange={(e) => setCogFilter(e.target.value as CogFilter)}
              className="px-3 py-2 rounded-lg bg-slate-50 ring-1 ring-slate-100 text-[13px] text-slate-700"
            >
      ...
      <div className="mt-6">
        <Card>
          ...
          <CardBody>
            {filtered.length === 0 ? (
              <EmptyState
                title={members.length === 0 ? '아직 등록된 어르신이 없어요' : '검색 결과가 없어요'}
              ...
              <>
                        ...
                        <tr key={m.id} className="hover:bg-slate-50 transition">
                          <td className="py-2.5 px-2">
                            <div className="flex items-center gap-2">
                              <div className="grid place-items-center w-7 h-7 rounded-full bg-slate-100 text-slate-600 text-[11px] font-bold shrink-0">
                                {m.name.charAt(0)}
                              </div>
                              <span className="font-semibold text-slate-900">{m.name}</span>
                            </div>
                          </td>
                          <td className="py-2.5 px-2 tabular-nums text-slate-700">{m.age}세</td>
                          <td className="py-2.5 px-2 text-slate-700">{cognitiveBadge(m.cognitiveLevel)}</td>
                          <td className="py-2.5 px-2 text-slate-500">{m.facility}</td>
                          <td className="py-2.5 px-2 text-slate-700">
                            {m.guardianName ? (
                              <>
                                {m.guardianName}
                                <span className="text-slate-400 text-[11px] ml-1">{m.guardianRelation}</span>
                              </>
                            ) : (
                              <span className="text-slate-300">미등록</span>
                            )}
                          </td>
                          <td className="py-2.5 px-2 tabular-nums text-slate-700">{m.sessionCount}</td>
                          <td className="py-2.5 px-2 text-slate-500 tabular-nums">{timeAgoKo(m.lastSessionAt)}</td>
                          <td className="py-2.5 px-2">
                            {m.inActiveSession ? (
                ...
                <ul className="md:hidden divide-y divide-slate-100 -mx-2">
                  {filtered.map((m) => (
                    <li key={m.id} className="py-3 px-2 flex items-center gap-3">
      ...
      <div className="mt-6">
        <Card>
          ...
          <CardBody>
            {guardians.length === 0 ? (
              <EmptyState title="아직 등록된 보호자가 없어요" hint="어르신 등록 시 함께 추가됩니다" />
            ) : (
              <ul className="divide-y divide-slate-100">
                {guardians.slice(0, 5).map((m) => (
                  <li key={m.id} className="py-2.5 flex items-center gap-3">
                    <div className="grid place-items-center w-8 h-8 rounded-full bg-slate-100 text-slate-600 text-[11px] font-bold shrink-0">
                      {m.guardianName?.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-semibold text-slate-800">
                        {m.guardianName}님 <span className="font-normal text-slate-400">{m.guardianRelation}</span>
                      </div>
                      <div className="text-[11px] text-slate-400 mt-0.5">→ {m.name} 어르신 · {m.facility}</div>
                    </div>
                  </li>
                ))}
                {guardians.length > 5 && (
                  <li className="py-3 text-center text-[12px] text-slate-400">+{guardians.length - 5}명 더</li>
                )}
              </ul>
            )}
          </CardBody>
